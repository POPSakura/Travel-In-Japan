    ...
    <div class="info-wrapper">
      ...
      <div class="series">
        <div class="image">
          ...
          <span class="title">{{ placeInfo.place }}</span>
        </div>
        <div class="detail">
          <p>{{ placeInfo.introduction }}</p>
          <div>
            <p>
...
export default {
  ...
  data() {
    return {
      placeID: parseInt(this.$route.query.id),
  ...
  methods: {
    commit(form) {
      this.$refs[form].validate(async valid => {
        if (valid) {
          const { placeID, kind, price, quantity, total } = this
          const form = JSON.parse(JSON.stringify(this.form))
          const { data } = await this.$axios.post('/order/addOrderOfPlace', Object.assign({}, form, {
            placeID, kind, price, quantity, total, place: this.placeInfo.place
          }))
          if (data.code === 0) {
            this.$message.success(`${data.msg}即将跳转至付款页面！`)
            setTimeout(() => {
              this.$router.push({ name: 'place-book-payment', params: { id: data.order_id } })
            }, 2000)
          } else {
    ...
    async getPlaceInfoOfBook() {
      const { data } = await this.$axios.get('/geo/getPlaceInfoOfBook', {
        params: { id: this.placeID }
      })
      if (data.code === 0) this.placeInfo = data.placeInfo
    },
    async checkStatus() {
      const { placeID } = this
      const { data } = await this.$axios.get('/order/checkPlaceOrderStatus', {
        params: { id: placeID }
      })
      if (data.code === -1) {
        this.$message.error(data.msg)
